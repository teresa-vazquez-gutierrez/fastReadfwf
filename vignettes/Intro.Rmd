...
Producing a data.table is now straightforward and fairly fast. The function `xlsxToSchema()` is used to load the schema object. Then, the function `fread_fwf()` is used to read the file with that schema.
...
data.DT.converted <- setTypes(data.DT, stSchema)
str(data.DT.converted)
```
...
data.DT.Char <- setChar(data.DT.converted)
...
```{r writeDT, echo = FALSE, eval = TRUE, cache = FALSE, dependson = 'setChar'}
system.time(
