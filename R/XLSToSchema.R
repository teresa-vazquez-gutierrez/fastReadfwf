...
#' XLSToSchema(
#'  file.path(path, 'EPA/stEPA2018_Schema.xlsx'),
#'  sheetname = 'stEPA2018_Schema',
#'  lang = 'en')
#' }
#'
#' @import data.table
...
XLSToSchema <- function(xlsname, sheetname, lang = 'sp'){
  ...
  if (lang == 'sp') {
    ...
    colnames(xls) <- c('variable', 'width', 'initialPos',
  ...
  if (all(is.na(xls$finalPos)) & all(is.na(xls$initialPos))) {
    ...
    xls$initialPos <- 1 + c(0, cumsum(xls$width)[-dim(xls)[1]])
    xls$finalPos <- xls$initialPos + xls$width - 1
